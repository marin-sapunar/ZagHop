...
def HARD_Real_to_Complex_SH(coeffs):
    ...
    if l==1:
        ...
        return coeffs
    return np.array(complex_coeffs)
def HARD_Complex_to_Real_SH(coeffs):
    real_coeffs = []
    l = (len(coeffs)-1)//2
    if l == 1:
        real_coeffs.append(-np.sqrt(2)*np.real(coeffs[0]))
...
#print(d_ORBS_SPHE_HARM_1)
print(transformation_matrix)
print("HERE",np.matmul(transformation_matrix.T,HARD_Real_to_Complex_SH(d_ORBS_SPHE_HARM_1))/np.sqrt(3))

rev_tranf = np.linalg.pinv(transformation_matrix.T)
print(HARD_Complex_to_Real_SH(np.matmul(rev_tranf,d_ORBS_1_GAUSS))*np.sqrt(3))#/np.sqrt(2*2-1))
