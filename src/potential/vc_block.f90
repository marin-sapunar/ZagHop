...
contains
    ...
    subroutine vc_block_allocate(self, max_order, nmode, ns1, ns2)
        class(vc_block) :: self
        ...
        self%ns1 = ns1
        self%ns2 = ns2
        if (max_order >= 0) allocate(self%zero(ns1, ns2), source=0.0_dp)
        if (max_order >= 1) allocate(self%linear(nmode, ns1, ns2), source=0.0_dp)
        if (max_order >= 2) allocate(self%quad(nmode, nmode, ns1, ns2), source=0.0_dp)
    end subroutine vc_block_allocate


    subroutine vc_block_check_order(self, order, nmode, ns1, ns2)
        class(vc_block) :: self
        ...
        if (self%max_order < 0) then
            call self%allocate(order, nmode, ns1, ns2)
            return
        end if
        
        if (self%nmode /= nmode .or. self%ns1 /= ns1 .or. self%ns2 /= ns2) then
        ...
        if (order >= 1) then
            allocate(self%linear(self%nmode, self%ns1, self%ns2), source=0.0_dp)
        end if
        if (order >= 2) then
            allocate(self%quad(self%nmode, self%nmode, self%ns1, self%ns2), source=0.0_dp)
        end if   
    ...
    subroutine vc_block_zero(self, nmode, zero)
        ...
        call self%check_order(0, nmode, size(zero, 1), size(zero, 2))
        self%zero = zero
    ...
    subroutine vc_block_linear(self, linear)
        ...
        call self%check_order(1, size(linear, 1), size(linear, 2), size(linear, 3))
        self%linear = linear
    ...
    function vc_block_eval(self, q) result(val)
        ...
        allocate(val(self%ns1, self%ns2), source=self%zero)
